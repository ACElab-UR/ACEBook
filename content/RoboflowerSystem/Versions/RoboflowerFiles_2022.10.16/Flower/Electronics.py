...
pi = pigpio.pi()  # the pi accesses the local pi's gpios
...
class Pump:
    def __init__(self, fwdpin, bkwpin, leftencoder, rightencoder, pigpiodaemon):
        ...
        self.stepsForTurn = 700  # nb of steps for one full motor turn (to check)
        self.UlForTurn = 6  # one motor turn delivers 6uL
    ...
    def do_deliver(self, volume, speed):
        print(volume, speed)  # so I know what happens
        self._killer = False
...
class Feeder:
    ...
    def do_fill(self, pump, amount= None, speed=255):
        if self.cleaned:
            self.emptyPump.deliver(-12000,255)  # pump 4 pumps back 12uL if cleaned (<=> if water pump delivered)
        if pump == 0:
    ...
    def (self):  # cleaning routine: what happens when the feeder is being cleaned
        self.waterPump.deliver(12000,255)  # pump 3 delivers 12uL
        self.emptyPump.deliver(-21000,255)  # pump 4 flushes 21uL
        self.waterPump.deliver(9000,255)  # pump 3 delivers 9uL
        self.cleaned=True
    ...
    def prime(self):
        for i in range(5):
            print('filling pump 0: delivers 20uL')
            self.deliverPump0.deliver(20000,255)
            print('done')
            print('flushing: pump 4 flushes 25uL')
            self.emptyPump.deliver(-25000,255)
            print('done')

            print('filling pump 1: delivers 20uL')
            ...
            print('filling waterPump: delivers 20uL')
