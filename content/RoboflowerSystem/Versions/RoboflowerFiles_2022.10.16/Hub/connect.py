...
#  This class provides all the necessary functions to connect to an MQTT broker + publish, subscribe, receive, etc.
class MQTT:
    ...
    def keepchecking(self):
        while True:
            if len(self.ONflowers) > 0:
                if time.time()-self.lastcheck > 60:  # checks every minute if flower is still up!
